...
use nebulafx_iam::error::is_err_no_such_user;
...
impl Operation for ListCannedPolicies {
    async fn call(&self, req: S3Request<Body>, _params: Params<'_, '_>) -> S3Result<S3Response<(StatusCode, Body)>> {
        ...
        let Ok(iam_store) = nebulafx_iam::get() else { return Err(s3_error!(InternalError, "iam not init")) };
...
impl Operation for SetPolicyForUserOrGroup {
    async fn call(&self, req: S3Request<Body>, _params: Params<'_, '_>) -> S3Result<S3Response<(StatusCode, Body)>> {
        ...
        iam_store
            .policy_db_set(&query.user_or_group, nebulafx_iam::store::UserType::Reg, query.is_group, &query.policy_name)
            .await
            .map_err(|e| {
