...
use nebulafx_s3select_query::get_global_db;
...
use nebulafx_zip::CompressionFormat;
...
static NEUBULAFX_OWNER: LazyLock<Owner> = LazyLock::new(|| Owner {
...
impl FS {
    ...
    async fn put_object_extract(&self, req: S3Request<PutObjectInput>) -> S3Result<S3Response<PutObjectOutput>> {
        ...
        let prefix = req
            .headers
            .get("X-Amz-Meta-NEUBULAFX-Snowball-Prefix")
        ...
        while let Some(entry) = entries.next().await {
            ...
            if let Ok(fpath) = f.path() {
                ...
                let event_args = nebulafx_notify::EventArgs {
        ...
        if let Some(alg) = &input.checksum_algorithm {
            if let Some(Some(checksum_str)) = req.trailing_headers.as_ref().map(|trailer| {
                let key = match alg.as_str() {
                    ChecksumAlgorithm::CRC32 => nebulafx_rio::ChecksumType::CRC32.key(),
                    ChecksumAlgorithm::CRC32C => nebulafx_rio::ChecksumType::CRC32C.key(),
                    ChecksumAlgorithm::SHA1 => nebulafx_rio::ChecksumType::SHA1.key(),
                    ChecksumAlgorithm::SHA256 => nebulafx_rio::ChecksumType::SHA256.key(),
                    ChecksumAlgorithm::CRC64NVME => nebulafx_rio::ChecksumType::CRC64_NVME.key(),
                    _ => return None,
                };
...
async fn get_validated_store(bucket: &str) -> S3Result<Arc<nebulafx_ecstore::store::ECStore>> {
    let Some(store) = new_object_layer_fn() else {
        return Err(S3Error::with_message(S3ErrorCode::InternalError, "Not init".to_string()));
    };
...
impl S3 for FS {
    ...
    async fn delete_object(&self, mut req: S3Request<DeleteObjectInput>) -> S3Result<S3Response<DeleteObjectOutput>> {
            ...
            || obj_info.version_purge_status == VersionPurgeStatusType::Pending
        {
            schedule_replication_delete(DeletedObjectReplicationInfo {
                delete_object: nebulafx_ecstore::store_api::DeletedObject {
    ...
    async fn delete_objects(&self, req: S3Request<DeleteObjectsInput>) -> S3Result<S3Response<DeleteObjectsOutput>> {
        ...
        struct DeleteResult {
            delete_object: Option<nebulafx_ecstore::store_api::DeletedObject>,
            error: Option<Error>,
        }
    ...
    async fn get_bucket_location(&self, req: S3Request<GetBucketLocationInput>) -> S3Result<S3Response<GetBucketLocationOutput>> {
        ...
        if let Some(region) = nebulafx_ecstore::global::get_global_region() {
    ...
    async fn get_object(&self, req: S3Request<GetObjectInput>) -> S3Result<S3Response<GetObjectOutput>> {
            ...
            && rs.is_none()
        {
            ...
            for (key, checksum) in checksums {
                ...
                match nebulafx_rio::ChecksumType::from_string(key.as_str()) {
                    nebulafx_rio::ChecksumType::CRC32 => checksum_crc32 = Some(checksum),
                    nebulafx_rio::ChecksumType::CRC32C => checksum_crc32c = Some(checksum),
                    nebulafx_rio::ChecksumType::SHA1 => checksum_sha1 = Some(checksum),
                    nebulafx_rio::ChecksumType::SHA256 => checksum_sha256 = Some(checksum),
                    nebulafx_rio::ChecksumType::CRC64_NVME => checksum_crc64nvme = Some(checksum),
                    _ => (),
                }
            }
        }
    ...
    async fn list_buckets(&self, req: S3Request<ListBucketsInput>) -> S3Result<S3Response<ListBucketsOutput>> {
        ...
        let output = ListBucketsOutput {
            ...
            owner: Some(NEUBULAFX_OWNER.to_owned()),
            ..Default::default()
        };
        Ok(S3Response::new(output))
    }
    ...
    async fn create_multipart_upload(
        ...
        let checksum_type = nebulafx_rio::ChecksumType::from_header(&req.headers);
        if checksum_type.is(nebulafx_rio::ChecksumType::INVALID) {
            return Err(s3_error!(InvalidArgument, "Invalid checksum type"));
        } else if checksum_type.is_set() && !checksum_type.is(nebulafx_rio::ChecksumType::TRAILING) {
            opts.want_checksum = Some(nebulafx_rio::Checksum {
                checksum_type,
                ..Default::default()
            });
        }
    ...
    async fn list_parts(&self, req: S3Request<ListPartsInput>) -> S3Result<S3Response<ListPartsOutput>> {
        ...
        let output = ListPartsOutput {
            ...
            initiator: Some(Initiator {
                id: NEUBULAFX_OWNER.id.clone(),
                display_name: NEUBULAFX_OWNER.display_name.clone(),
            }),
    ...
    async fn put_bucket_notification_configuration(
        ...
        let region = nebulafx_ecstore::global::get_global_region().unwrap_or_else(|| req.region.clone().unwrap_or_default());
