use nebulafx_appauth::token::Token;
...
pub fn init_license(license: Option<String>) {
    ...
    let token = nebulafx_appauth::token::parse_license(&license).unwrap_or_default();
...
pub fn license_check() -> Result<()> {
    ...
    let invalid_license = LICENSE.get().map(|token| {
        if token.expired < SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs() {
            ...
            return Err(Error::other("Incorrect license, please contact NebulaFX."));
        }
    ...
    //         return Err(Error::other("Incorrect license, please contact NebulaFX.".to_string()));
    //     }
